#!/bin/bash

rm bin/experiments_ghost
GHOST=/home/flo/ghost

# Without trace
g++ -std=c++11 -Ofast -DNDEBUG experiments/main_ghost.cpp $GHOST/src/variables/variable.cpp $GHOST/src/variables/building.cpp $GHOST/src/constraints/wallinConstraint.cpp $GHOST/src/domains/wallinDomain.cpp $GHOST/src/objectives/wallinObjective.cpp -o bin/experiments_ghost


# With trace
g++ -std=c++11 -Ofast experiments/main_ghost.cpp $GHOST/src/variables/variable.cpp $GHOST/src/variables/building.cpp $GHOST/src/constraints/wallinConstraint.cpp $GHOST/src/domains/wallinDomain.cpp $GHOST/src/objectives/wallinObjective.cpp -o bin/experiments_ghost

